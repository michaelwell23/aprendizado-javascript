...
setTimeout(() => {
  console.log('Código assíncrono executada!');
}, 2000);
...
*/
const argumentsInFunction = (value1, value2, value3) => {
  if ([value1, value2, value3].includes(undefined)) {
    return 'A função deve ser invocada com 3 argumentos';
  }

  return 'A string foi invocada com 3 argumentos';
};

console.log(argumentsInFunction(2, 'Michael', () => {}));
...
booksBox.addBooks = (booksQuantity) => {
  ...
  const boxSpaceAreNotEnough = booksBox.booksIn + booksQuantity > spaces
  ...
  return `Há ${booksBox.booksIn} ${bookPluralOrSingular} na caixa`;
};
