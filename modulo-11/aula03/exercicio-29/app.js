...
getPokeDex('bulbasaur', (error, data) => {
  logPokemonData(error, data)
  getPokeDex('charmander', (error, data) => {
    logPokemonData(error, data)
    getPokeDex('squirtle', (error, data) => {
      logPokemonData(error, data)
    })
  })
})
...
console.log('EXERCÍCIO 02', map([1,2,3], item => item * 2))
console.log('EXERCÍCIO 02', map([1,2,3], item => item * 3))
...
console.log("EXERCÍCIO 03:",person.getName())
...
console.log('EXERCÍCIO 05:', getFullName({ firstName: 'Afonso', lastName: 'Solano' }))
...
return colorHex[color]
  ? console.log(`O hexadecimal para a cor ${color} é ${colorHex[color]}`)
  : console.log(`Não temos o equivalente hexadecimal para ${color}`)
}
const colors = ['azul', 'preto', 'vermelho', 'rosa', 'amarelo', 'roxo', 'marron']
const logColorMessage = color => convertToHex(color)
...
console.log('EXERCÍCIO 07:', agesFrequency)