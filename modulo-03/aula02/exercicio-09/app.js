...
console.log('EXERCÍCIO 01: ',convertToString(23));
...
*/
const numberOfString = string => string.length;
const characterAmount = numberOfString('pneumoultramicroscopicossilicovulcanoconiótico')
console.log('EXERCÍCIO 02: ', characterAmount)
...
const convertToLowerCase = string => string.toLowerCase();
  console.log('EXERCICIO 03:', convertToLowerCase('CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO'))
...
console.log(`EXERCÍCIO 04: ${getIndex('e', 'michael')}`)
...
*/
const existsItemIncluded = (item, array) => array.includes(item);
console.log('EXERCÍCIO 05: ', existsItemIncluded('Maçã', ['Pera', 'Uva', 'Maçã', 'Abacaxi']));
...
console.log(`EXERCÍCIO 06: `, arrayConcat([10,11,12], [13,14,15]))
...
const removeItemInArray = removeLastItem(['Michael', 'Gabriella', 'João', 'Marcela'])
console.log(`EXERCÍCIO 07: ${removeItemInArray}`)
...
console.log(`EXERCÍCIO 08: `, isNull(null))
...
const functionReturn  = callback => {
  callback()
}
const myName = () => {
  console.log('EXERCÍCIO 09: Michael Wellington Lopes')
}
functionReturn(myName)
...
console.log('EXERCÍCIO 10:', callCallBack(11, triploNumber));
;
...
numbers.forEach((index, element, array) => {
  const itemPosition = element + 1
  const sentence = `O ${itemPosition}º item do array [${array}] é ${index}`
  console.log(`EXERCÍCIO 11: ${sentence}`)
}) 
...
console.log('EXERCÍCIO 12:', letters)
...
const getLikesMessage = (names = []) => {
  ...
  const totalNameMinesTwo = names.length -2
  ...
  switch (names.length){
    ...
    case 3:   
      return `${firstName}, ${secondName} ${thirdName} curtiram isso`
   default:
      return `${firstName}, ${secondName} e ${totalNameMinesTwo} curtiram isso`  
  }
}

console.log(getLikesMessage(['Michael', 'Gabriella', 'Vinicius', 'Heloisa']))