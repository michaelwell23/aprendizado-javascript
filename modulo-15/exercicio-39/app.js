...
const sum = (...args) => args.reduce((acc, doc) => (acc += doc), 0);
console.log('EXERCÍCIO 01:', sum(...numbers));
...
const closeAccordionItem = (accordionHeaderToBeClosed) => {
  ...
  const accodionBodyToBeClosed = document.querySelector(
    `[data-accordion-body="${accordionHeaderId}"]`
  );

  accordionHeaderToBeClosed.classList.remove('active');
  accodionBodyToBeClosed.classList.remove('active');
};
...
console.log('EXERCÍCIO 03', amarok);
console.log('EXERCÍCIO 03', jetta);
...
console.log('EXERCÍCIO 04:', getIndexesOfCharacter(aString, 'b'));
...
console.log('EXERCÍCIO 06:', correctDataFormat)
